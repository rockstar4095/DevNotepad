...
class HeadersPart(
    ...
    private suspend fun isHeadersTableEmpty(): Boolean {
        return articleContentViewModel.articlesContentRepository.getArticleHeadersSync(articleId).isEmpty()
    }
    ...
    private suspend fun matchHeadersFromServerAndLocal(headersFromServer: List<ArticleHeader>) {
        ...
        for (header in articleContentViewModel.articlesContentRepository.getArticleHeadersSync(articleId)) {
